...
use move_package_alt::{
    ...
    flavor::MoveFlavor,
    package::{Package, RootPackage, paths::PackagePath, root_package},
...
pub async fn compile<F: MoveFlavor>(
    ...
    if let Some(dependency_graph) = &root_pkg.dependencies().get(env) {
        ...
        let sui_mode = build_config.default_flavor == Some(move_compiler::editions::Flavor::Sui);
        ...
        let mut compiler = Compiler::from_package_paths(vfs_root, dependencies_paths, vec![])
            .unwrap()
            .set_flags(compiler_flags(&build_config));
        if sui_mode {
...
pub fn build_all<W: Write, F: MoveFlavor>(
    ...
    let (file_map, all_compiled_units) =
        build_for_driver(w, vfs_root, build_config, env, root_pkg, |compiler| {
            let compiler = compiler.add_save_hook(&program_info_hook);
...
pub(crate) fn build_for_driver<W: Write, T, F: MoveFlavor>(
    ...
    if let Some(dependency_graph) = &root_pkg.dependencies().get(&env) {
        ...
        Ok(compiler_driver(compiler)?)
    } else {
        bail!("Cannot compile package due to missing {env} environment")
    }
}
