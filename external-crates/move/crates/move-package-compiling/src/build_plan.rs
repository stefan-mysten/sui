...
use crate::{
    ...
    compiled_package::{self, CompiledPackage, build_all, build_for_driver},
};
use move_compiler::{
    Compiler,
    compiled_unit::AnnotatedCompiledUnit,
    shared::{PackagePaths, SaveFlag, SaveHook, files::MappedFiles},
};
...
impl<F: MoveFlavor> BuildPlan<F> {
    pub fn create(root_pkg: RootPackage<F>, build_config: &BuildConfig) -> PackageResult<Self> {
        let deps = root_pkg.dependencies().clone();
        let graph = deps
            .get(
                &build_config
                    .environment
            ...
            .ok_or_else(|| PackageError::Generic("Error".to_string()))?;
    ...
    pub fn compile_with_driver<W: Write>(
        ...
        let package_name = self.root_pkg.package_name().clone();
