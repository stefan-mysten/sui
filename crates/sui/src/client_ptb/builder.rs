...
use crate::{
    client_commands::upgrade_package,
    client_ptb::{
...
use move_package_compiling::{build_config::BuildConfig as MoveBuildConfig, compile_package};
    ...
    async fn handle_command_(
        ...
        match command {
            ...
            ParsedPTBCommand::Publish(sp!(pkg_loc, package_path)) => {
                let build_config = move_package_compiling::build_config::BuildConfig::default();
                let mut stdout = std::io::stdout();
                let mut writer: &mut dyn std::io::Write = &mut stdout;
                let compiled_package = compile_package::<&mut dyn std::io::Write, SuiFlavor>(
