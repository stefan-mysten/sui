...
use sui_faucet_proxy::{create_wallet_context, start_faucet, AppState};
use sui_faucet_proxy::{FaucetConfig, LocalFaucet};
use tracing::info;
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let app_state = Arc::new(AppState {
        faucet: LocalFaucet::new(context, &prometheus_registry, config.clone())
            .await
            .unwrap(),
        config,
    });
