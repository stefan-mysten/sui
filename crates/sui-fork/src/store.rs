...
///
/// Steady-state reads and writes go through this type's policy methods and
/// trait impls, which own the resolution order and the write-back rules.
/// Creation-time workflows (startup persistence and seeding) instead reach the
/// components directly through [`Self::remote`], [`Self::local_store`], and
/// [`Self::metadata`], because those flows have no fallback policy to apply
/// and forwarding them here would only mirror the component APIs.
#[derive(Clone)]
