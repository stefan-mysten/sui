...
//! - [`CheckpointStore`] - Retrieve verified checkpoint summaries by sequence number or digest
...
//! - [`stores::ForkingStore`] - Primary/secondary composition for cached reads
//!
//! ## Composition
//!
//! Use [`stores::ForkingStore`] to compose a writable primary cache over a
//! read-only source.
...
type CheckpointData = VerifiedCheckpoint;
...
/// Retrieve verified checkpoint summaries and indexes.
pub trait CheckpointStore {
    /// Return a verified checkpoint summary by sequence number.
    fn get_checkpoint_by_sequence_number(
        &self,
        sequence: CheckpointSequenceNumber,
    ) -> Result<Option<CheckpointData>, Error>;

    /// Return the latest verified checkpoint summary known to this store.
...
pub trait CheckpointStoreWriter: CheckpointStore {
    /// Persist a verified checkpoint summary.
