...
pub struct CompiledPackage {
    ...
    pub dependency_graph: Vec<Arc<Package<SuiFlavor>>>,
}
...
impl BuildConfig {
    ...
    pub fn build(self, path: &Path) -> anyhow::Result<CompiledPackage> {
        // TODO: these flags are not taken into consideration yet. Fix it!
        ...
        let dependency_graph = root_pkg.dependencies();

        let package = tokio::task::block_in_place(|| {
            let mut stdout = std::io::stdout();
            let mut writer: &mut dyn Write = &mut stdout;
            tokio::runtime::Handle::current().block_on(move_package_alt_compilation::compile_from_root_package::<
                &mut dyn Write,
                SuiFlavor,
            >(
                root_pkg, &self.config, &mut writer
            ))
        })?;
